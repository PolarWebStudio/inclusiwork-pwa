...
DB_NAME = "database.db"
OGADS_API_KEY = os.environ.get('OGADS_API_KEY', '')
...
# ==========================================
# ENDPOINT FILTRADO RIGUROSO OGADS
# ==========================================
...
def get_ogads_offers():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Lista ampliada de términos engañosos, sorteos, encuestas sueltas y bonos
            blacklisted_terms = [
                'gira', 'rueda', 'wheel', 'spin', 'bono', 'bonus', 'reclamar', 'claim',
                'samsung', 'iphone', 'playstation', 'ps5', 'ganar', 'win', 'sorteo',
                'encuesta', 'survey', 'predict', 'score', 'marca goles', 'atrapas',
                'monedas', 'coins', 'catch', 'cuestionario', 'dinero en linea',
                'unlock', 'desbloquear', 'toca para', 'tap to', 'click to', 'haz clic'
            ]

            filtered_offers = []
            for offer in raw_offers:
                title = str(offer.get('name', '') or offer.get('name_short', '')).lower()
                desc = str(offer.get('ad_description', '') or offer.get('instructions', '')).lower()

                # Si el título o la descripción contienen alguno de los términos no deseados, se ignora
                if any(term in title for term in blacklisted_terms):
                    continue
                if any(term in desc for term in blacklisted_terms) and 'app' not in desc and 'instal' not in desc:
                    continue
            ...
            return jsonify({
                "error": "Error al consultar la API de OGAds",
